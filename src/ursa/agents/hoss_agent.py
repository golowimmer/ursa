...
class HOSSState(TypedDict, total=False):
    ...
    # Data conversion keys
    lammps_to_json_returncode: Optional[int]
    lammps_to_json_stdout: str
    lammps_to_json_stderr: str
...
class HOSSAgent(BaseAgent):
    def __init__(
        ...
        **kwargs,
        ...
        # If none, agent will pick a name
        ...
        self.check_input_chain = (
            ChatPromptTemplate.from_template(
                ...
                "requires running HOSS. Decide if this involves using "
                "Lammps-based data. Further, check if the task requires "
                "building and/or analysing post-processing files that create "
                "metrics for existing simulation output."
        ...
        # Chain: check for LAMMPS file
        self.check_lammps_path_chain = (
            ChatPromptTemplate.from_template(
                "Consider this task: {simulation_task}. You decided this task "
                "involves Lammps-based data. Check which path this data is in, "
                "with a default as `data/materials` if no path for this is "
                "specified in the task, and a default file name null if no "
                "file name is specified."
                "Use this exact scheme in your response:"
                "{{\n"
                '  "lammps_path": <str>\n'
                '  "lammps_data_name": <str> or null\n'
                "}}\n"
            )
            | self.llm
            | self.str_parser
        ...
        # Chain: set LAMMPS input file name if none was given
        self.get_lammps_name_chain = (
            ChatPromptTemplate.from_template(
                "Consider this task: {simulation_task}. You decided this task "
                "involves Lammps-based data, but the data file name was "
                "unclear. Given the files {files}, pick one that could be "
                "suitable for the task. In your response, either reply exactly" \
                "with the file name, or with None."
            )
            | self.llm
            | self.str_parser
        )

        # Chain: set LAMMPS to json output file name if none was given
        self.check_lammps_out_name = (
            ChatPromptTemplate.from_template(
                "Consider this task: {simulation_task}. You decided this task "
                "involves Lammps-based data, which we will need to convert to "
                "HOSS input data in json format. We need to pick a file name "
                "for the converted HOSS input data file. Given the simulation "
                "task and the files `{files}` in the data folder, pick a new "
                " name for the converted json file that could be suitable for "
                "the task. In your response, reply exactly with <file_name>.json, "
                "where <file_name> is your chosen name."
            )
            | self.llm
            | self.str_parser
        )
        ...
        update_config_str = (
            ...
            "including in particular any entries related to samples. Also make "
            "Sure to set the entry `work_dir_name` to correspond to your "
    ...
    def plot_graph(self):
        ...
        ###
        print("Plotting HOSS graph and exiting here.")
        exit()
        # # #
    ...
    def _check_input_task(self, state: HOSSState) -> HOSSState:
        ...
        if '"run_HOSS": true' in required_data:
            state["run_HOSS"] = None
    ...
    def _check_HOSS_pipeline(self) -> str:
        ...
        if not missing_config:
            ...
            try:
                Hoss_path = config["HOSS_executable_path"]
            ...
            try:
                cubit_path = config["cubit_executable_path"]
    ...
    def _simulate_or_analyze(self, state: HOSSState) -> str:
        ...
        if "Lammps_data" in state and "choice_json" not in state:
            return "convert_Lammps_data"
        if "run_HOSS" in state:
    ...
    def _convert_LAMMPS_data(self, state: HOSSState) -> HOSSState:
        ...
        # Check if we can find the lammps text file
        lammps_file_data = json.loads(self.check_lammps_path_chain.invoke({
            "simulation_task": state["simulation_task"]}))
        #TODO: Check if lammps file path exists

        files = []
        for f_name in os.listdir(lammps_file_data["lammps_path"]):
            files.append(f_name)

        if lammps_file_data["lammps_data_name"] is None:
            lammps_file_data["lammps_data_name"] = \
                self.get_lammps_name_chain.invoke( \
                {"simulation_task": state["simulation_task"], "files": files})
            if lammps_file_data["lammps_data_name"] is None:
                raise RuntimeError("Could not find a lammps file given "
                                   "simulation task.")

        path = Path(lammps_file_data["lammps_path"],
                    lammps_file_data["lammps_data_name"])
        if not path.exists():
            raise RuntimeError(f"Could not find lammps file {path} related to "
                               "simulation task; exiting here.")

        # Check if specified converted lammps file name is None or exists
        out_path = None
        if self.lammps_to_json_name is not None:
            out_path = Path(lammps_file_data["lammps_path"],
                            self.lammps_to_json_name)
        if self.lammps_to_json_name is None or not out_path.exists():
            self.lammps_to_json_name = \
                self.check_lammps_out_name.invoke({
                    "simulation_task": state["simulation_task"],
                    "files": files})

        # Convert lammps file to json
        result = subprocess.run(["python", \
            f"{self.src_dir}/{HOSSSourceFiles.text_to_json}.py", \
            "--Json_name", self.lammps_to_json_name,
            "--text_files", lammps_file_data["lammps_data_name"],
            "--data_dir", lammps_file_data["lammps_path"]])

        if result.returncode == 0:
            print("Transferred Lammps output to json data format.")
        else:
            # TODO: catch errors from running this file
    ...
    def _prepare_HOSS_batch_runs(self, state: HOSSState) -> HOSSState:
        ...
        lammps_data_info = ""
        if "lammps_to_json_returncode" in state:
            lammps_data_info = ("Note that the config file contains an entry "
                "`materials_json_name`. Earlier, you converted Lammps data "
                f"to a json file named {self.lammps_to_json_name}. Use this "
                "json file name in your modified config file.")
        ...
        if state.get("batch_job_creation_attempts", 0) == 0:
            ...
            llm_yaml_text = self.update_config_file.invoke({
                ...
                "lammps_data_info": lammps_data_info})
        else:
        ...
        # If requiring more than one attempt, retain previous error messages
        # to avoid agent alternating between two separate errors
        if state.get("batch_job_creation_attempts", 0) == 0:
            errors = result.stderr
        else:
            errors = state["batch_create_stderr"] \
                + b' Error: ' + result.stderr
    ...
    def _HOSS_batch_setup_success(self, state: HOSSState) -> str:
        ...
        print("Failed setting up batch jobs; trying again.")
        #TODO: add verbosity clause here
        print(f"Error message: {state['batch_create_stderr']}")
    ...
    def _analysis_success(self, state: HOSSState) -> str:
        ...
        print("Failed setting up analysis; trying again.")
        #TODO: add verbosity clause here
        print(f"Error message: {state['analysis_stderr']}")
